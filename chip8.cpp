...
void Chip8::LoadROM(char const *filename) {
  ...
  try {
    if (file.is_open()) {
      // Get size of file and allocate a buffer to hold the contents (this can
      // be done due to "ate" flag which moves the file pointer to end of file)
...
Chip8::Chip8() {
  ...
  // Can point to another table for further deconstruction if the opcode is not
  // unique due to the first hexa number.
...
void Chip8::OP_3xkk() {
  ...
  if (registers[Vx] == byte) {
    // Size of the instruction is 2 Bytes => in order to skip an instruction we
